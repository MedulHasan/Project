...
if dataset is not None:
    ...
    def load_data(data):
        file_name = 'data/' + data
        file = open(file_name).readlines()[1:]
        return file
    ...
    if section == "Deep Learning":
        model_name = st.sidebar.selectbox("Select Model", ("LSTM", "CNN"))
        window_size = 2
        ...
        # sample = df.shape[0]
        #
        # data = df.filter(['CLOSEP*'])
        # data = data.replace('[^\d.]', '', regex=True).astype(float)
        # close = data.values
        #
        # X = np.atleast_3d(np.array([close[i:i + window_size] for i in range(0, sample - window_size)]))
        # Y = close[window_size:]
        ...
        def CNN_MODEL(window_size):
            model = Sequential((
                Conv1D(input_shape=(window_size, 1), kernel_size=5, activation="relu", filters=64,
                       padding='causal'),
                MaxPooling1D(),
                Conv1D(kernel_size=5, activation="relu", filters=64, padding='causal'),
                # MaxPooling1D(),
                Flatten(),
        ...
        def fit_model(model_name):
            ...
            if model_name == "CNN":
                ...
                st.write("Mean Absolute Error:", score[1])
